...
export default function Level1() {
  ...
  // Use the input mask hook for description field (max 50 characters)
  const [description, handleDescriptionChange, setDescription] = useInputMask('', 'description');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (json.length === 0) {
        setMessage({
          type: "info",
          text: "No Level 1 accounts found. You can add one using the form above."
        });
      }
    } catch (err) {
      setMessage({
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editId 
      ? `http://localhost:5000/api/accounts/level1/${editId}` 
      : `http://localhost:5000/api/accounts/level1`;
  ...
  const handleEdit = (item) => {
    // Set the values using the setter functions from the hook
    setCode(item.code);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this account?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/accounts/level1/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        ...
        if (!res.ok) {
          // Check if the error is about child accounts
          if (json.error && json.error.includes('child accounts')) {
            // Show a more specific error message with the child count
            const childCount = json.childCount || 0;
            setMessage({
              type: "error",
              text: `Cannot delete this account. It has ${childCount} child account(s) in Level 2. Please delete the child accounts first.`
            });
          } else {
      ...
      <div className="ml-0 md:ml-64 transition-all duration-300 fixed inset-0 overflow-hidden">
        <div className="max-w-4xl mx-auto mt-6 p-4 md:p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg flex flex-col h-[calc(100vh-3rem)] md:h-[calc(100vh-3rem)]">
          <h2 className="text-xl md:text-2xl font-bold text-center text-blue-600 dark:text-blue-400 mb-4 md:mb-6 flex items-center justify-center gap-2">
            ...
            <span className="truncate">Level 1 Chart of Accounts</span>
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6 flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder="Assets (max 50 characters)"
                    className={inputClass}
                    required
                  />
                  <div className="text-xs text-gray-500 absolute right-3 top-3">
            ...
            <button
              type="submit"
              disabled={loading || !companyId || !username || code.length !== 2 || description.length === 0}
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar responsive-table">
                {data.length > 0 ? (
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No Level 1 accounts found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
