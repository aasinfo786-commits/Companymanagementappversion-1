...
export default function Level3() {
  ...
  // Use the input mask hook for code field (3 digits only)
  const [code, handleCodeChange, setCode] = useInputMask('', 'code3');
  ...
  const fetchLevel3Accounts = async () => {
    ...
    try {
      ...
      if (json.data && json.data.length === 0) {
        setMessage({
          type: "info",
          text: "No Level 3 accounts found for this combination. You can add one using the form above."
        });
      }
    } catch (err) {
      setMessage({
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editId 
      ? `http://localhost:5000/api/accounts/level3/${editId}` 
      : `http://localhost:5000/api/accounts/level3`;
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this account?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/accounts/level3/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        ...
        if (!res.ok) {
          // Check if this is a "has children" error
          if (json.hasChildren || (json.error && json.error.includes('child accounts'))) {
            // Show a more specific error message with the child count
            const childCount = json.childCount || 0;
            setMessage({
              type: "error",
              text: `Cannot delete this account. It has ${childCount} child account(s) in Level 4. Please delete the child accounts first.`
            });
          } else {
...
{/* Selects for Level 1 and Level 2 */}
...
{/* Inputs for code, title, and balance */}
<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
  ...
  <div className="flex flex-col">
    ...
    />
            ...
            <button
              type="submit"
              disabled={loading || !companyId || !username || !level1Id || !level2Id || code.length !== 3 || title.length === 0}
          ...
          {companyId && level1Id && level2Id ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar">
                {data.length > 0 ? (
                  <table className="w-full">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200">
                        ...
                        <th className="p-3 text-left font-semibold text-sm">Parent Accounts</th>
                    ...
                    <tbody>
                      {data.map((item) => {
                        const parentLevel1 = level1Accounts.find(a => a._id === item.level1Id);
                        const parentLevel2 = level2Accounts.find(a => a._id === item.level2Id);
                        return (
                          <tr
                            key={item._id}
                            id={`row-${item._id}`}
                          ...
                          >
                            <td className="p-3 font-mono text-sm">{item.parentLevel1Code}{item.parentLevel2Code}{item.code}</td>
                            <td className="p-3 text-sm">{item.title}</td>
                            <td className="p-3 font-mono text-sm">{item.balance.toFixed(2)}</td>
                            <td className="p-3 text-sm">
                              <div className="text-xs">
                                <div>L1: {parentLevel1?.code} - {parentLevel1?.description}</div>
                                <div>L2: {parentLevel2?.code} - {parentLevel2?.title}</div>
                              </div>
                            </td>
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No Level 3 accounts found for this combination. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              {!companyId 
                ...
                : "Please select a Level 2 account to view Level 3 accounts"}
            </div>
          )}
        </div>
      </div>
