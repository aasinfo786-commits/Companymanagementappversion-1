...
export default function ImportChildCenterCoding() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/import-childcenter-coding/child-center`, {
        method: 'POST',
        body: formDataObj
      });
            ...
            <div className="max-h-40 overflow-y-auto text-xs text-gray-700 dark:text-gray-300 space-y-1">
              {data.duplicates.map((dup, index) => (
                <div key={index} className="p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded border border-yellow-100 dark:border-yellow-800">
                  ...
                  <div>Farm Code: {dup.row.farmCode}</div>
                  <div>Farm Subcode: {dup.row.farmSubCode}</div>
                  <div>Farm Title: {dup.row.farmTitle}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Existing record: {dup.existingRecord.parentCode}-{dup.existingRecord.childCode} ({dup.existingRecord.title})
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-purple-500 dark:text-purple-400">
            Import Child Center data from Excel files
          </p>
        </div>
            ...
            <div className={sectionClass}>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <div className="text-xs text-blue-600 dark:text-blue-400">
                  Your Excel file must contain the following columns: <span className="font-mono">farmCode</span>, <span className="font-mono">farmSubCode</span>, and <span className="font-mono">farmTitle</span>
                </div>
              </div>
            </div>
        ...
        {!companyId && (
          <div className="mt-4 p-4 text-center text-purple-500 dark:text-purple-400 bg-white dark:bg-gray-800 rounded-lg shadow border border-purple-200 dark:border-purple-700 text-sm">
            Please select a company to import Child Center data
          </div>
        )}
      </div>
    </div>
