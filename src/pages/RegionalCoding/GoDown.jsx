...
export default function GoDown() {
  ...
  const fetchData = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`http://localhost:5000/api/godown/${companyId}`);
      ...
      if (json.length === 0) {
        setMessage({
          type: "info",
          text: "No godowns found. You can add one using the form above."
        });
      }
    } catch (err) {
      setMessage({
  ...
  const handleSubmit = async (e) => {
    ...
    let codeNum;
    if (editId) {
      // When editing, use the value from the input
      codeNum = parseInt(code);
      if (isNaN(codeNum) || codeNum <= 0) {
        setMessage({
          type: "error",
          text: "Code must be a positive number"
        });
        return;
      }
    } else {
      // When creating, generate the next code
      codeNum = generateNextCode();
    }
    
    if (!/^[A-Za-z]$/.test(alphabet)) {
      setMessage({
        type: "error",
        text: "Alphabet must be a single character"
      });
      return;
    }
    ...
    const payload = { 
      companyId, 
      code: codeNum,  // Now sending as number
      name, 
      alphabet,
      updatedBy: username,
      updatedAt: now
    ...
    // Add created information for new godowns
    if (!editId) {
    ...
    const url = editId 
      ? `http://localhost:5000/api/godown/${editId}` 
      : `http://localhost:5000/api/godown`;
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: editId ? "Godown updated successfully!" : "Godown created successfully!"
      });
  ...
  const handleEdit = (item) => {
    // Convert code to string for the input field
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this godown?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/godown/${id}`, {
          method: 'DELETE'
        });
        const json = await res.json();
        ...
        setMessage({
          type: "success",
          text: "Godown deleted successfully!"
        });
        fetchData();
      } catch (err) {
        setMessage({
          type: "error",
          text: err.message || 'Failed to delete godown'
        });
      } finally {
        setLoading(false);
      }
      ...
      <div className="ml-0 md:ml-64 transition-all duration-300 fixed inset-0 overflow-hidden">
        <div className="max-w-5xl mx-auto mt-6 p-4 md:p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg flex flex-col h-[calc(100vh-3rem)]">
          <h2 className="text-xl md:text-3xl font-bold text-center text-blue-600 dark:text-blue-400 mb-4 md:mb-6 flex items-center justify-center gap-2">
            ...
            <span className="truncate">Godown Management</span>
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6 flex-shrink-0">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  placeholder={editId ? code : generateNextCode().toString()}
              ...
              <div className="flex items-center gap-2">
                <Home className="text-blue-500 w-4 h-4 flex-shrink-0" />
                <input
                  type="text"
                  value={name}
              ...
              <div className="flex items-center gap-2">
                <Type className="text-blue-500 w-4 h-4 flex-shrink-0" />
                <input
                  type="text"
                  value={alphabet}
            ...
            >
              ...
              {editId ? 'Update Godown' : 'Create Godown'}
            </button>
          </form>
          ...
          {companyId ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar responsive-table">
                {data.length > 0 ? (
                  <table className="w-full">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200">
                        ...
                        <th className="p-3 text-left font-semibold text-sm">Alphabet</th>
                    ...
                    <tbody>
                      {data.map((item) => (
                        ...
                        >
                          ...
                          <td className="p-3 text-sm">{item.alphabet}</td>
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No godowns found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view godowns
            </div>
          )}
        </div>
      </div>
