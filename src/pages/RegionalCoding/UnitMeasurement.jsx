...
export default function UnitMeasurement() {
  ...
  const fetchData = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`http://localhost:5000/api/unit-measurement/${companyId}`);
      ...
      if (json.length === 0) {
        setMessage({
          type: "info",
          text: "No units found. You can add one using the form above or import from FBR."
        });
      }
    } catch (err) {
      setMessage({
  ...
  const fetchFBRUnits = async () => {
    ...
    try {
      ...
      if (units.length === 0) {
        throw new Error('No units returned from FBR');
      }
      
      // Process the FBR data with correct field names
      const savePromises = units.map(unit => {
        const payload = {
          companyId,
          code: unit.uoM_ID.toString().padStart(2, '0'), // Ensure 2-digit code
          title: unit.description,
          createdBy: username,
          updatedBy: username
        };
        return fetch('http://localhost:5000/api/unit-measurement', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
      ...
      if (newIds.length > 0) {
        setNewlyAddedId(newIds[0]); // Highlight the first imported unit
      }
      
      setMessage({
        type: "success",
        text: `${units.length} units imported successfully from FBR!`
      });
      ...
      setMessage({
        type: "error",
        text: err.message || 'Failed to fetch units from FBR'
      });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^\d{2}$/.test(code)) {
      setMessage({
        type: "error",
        text: "Code must be exactly 2 digits"
      });
      return;
    }
    ...
    const unitCode = editId ? code : generateNextCode();
    const now = new Date();
    const payload = { 
      companyId, 
      code: unitCode, 
    ...
    // Add created information for new units
    if (!editId) {
    ...
    const url = editId 
      ? `http://localhost:5000/api/unit-measurement/${editId}` 
      : `http://localhost:5000/api/unit-measurement`;
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: editId ? "Unit updated successfully!" : "Unit created successfully!"
      });
...
const handleDelete = async (id) => {
  if (window.confirm('Are you sure you want to delete this unit?')) {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/unit-measurement/${id}`, {
        method: 'DELETE',
        headers: { 
          "Content-Type": "application/json",
      ...
      if (!res.ok) {
        ...
        if (data.details && data.details.references) {
          ...
          const referenceDetails = data.details.references
            .map(ref => `${ref.model}: ${ref.count} records with ${ref.field}`)
            .join(', ');
          
          throw new Error(`Cannot delete unit. Found references in: ${referenceDetails}. ${data.details.actionRequired || ''}`);
        } else {
          // Use the error message from the backend if available
          throw new Error(data.message || data.error || "Failed to delete unit");
        }
      }
      
      setMessage({
        type: "success",
        text: data.message || "Unit deleted successfully!"
      });
      fetchData();
    } catch (err) {
      setMessage({
        type: "error",
        text: err.message || 'Failed to delete unit'
      });
    } finally {
      setLoading(false);
      ...
      <div className="ml-0 md:ml-64 transition-all duration-300 fixed inset-0 overflow-hidden">
        <div className="max-w-5xl mx-auto mt-6 p-4 md:p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg flex flex-col h-[calc(100vh-3rem)]">
          <h2 className="text-xl md:text-3xl font-bold text-center text-blue-600 dark:text-blue-400 mb-4 md:mb-6 flex items-center justify-center gap-2">
            ...
            <span className="truncate">Unit of Measurement</span>
          </h2>
          ...
          {companyId ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar responsive-table">
                {data.length > 0 ? (
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No units found. Add one using the form above or import from FBR."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view units
            </div>
          )}
        </div>
      </div>
