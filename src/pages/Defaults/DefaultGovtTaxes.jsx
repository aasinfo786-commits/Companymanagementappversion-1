...
export default function DefaultGovtTaxes() {
  ...
  const fetchDefaultTaxes = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch default tax accounts');
      }
      const responseData = await res.json();
      const taxesData = Array.isArray(responseData) ? responseData : (responseData.data || []);
      setDefaultTaxes(taxesData);
      if (taxesData.length === 0) {
        setMessage({
          type: "info",
          text: "No default government tax accounts found. You can add one using the form above."
        });
      }
    } catch (err) {
  ...
  // Fetch level 4 accounts for all taxes to get their titles
  const fetchTitlesForTaxes = async () => {
    if (defaultTaxes.length === 0) return;
    
    try {
      // Get all unique level 4 IDs from taxes
      const level4Ids = [...new Set(defaultTaxes.map(tax => tax.level4Id))];
      ...
      // Update taxes with titles
      setDefaultTaxes(prev => 
        prev.map(tax => ({
          ...tax,
          level4Title: level4AccountMap[tax.level4Id] || tax.level4Title || 'Unknown Title'
        }))
      );
    } catch (err) {
      console.error('Error fetching titles for taxes:', err);
      // If fetching fails, keep existing titles or set to 'Unknown Title'
      setDefaultTaxes(prev => 
        prev.map(tax => ({
          ...tax,
          level4Title: tax.level4Title || 'Unknown Title'
        }))
      );
    }
  };
  ...
  useEffect(() => {
    if (newlyAddedId && tableRef.current) {
      const element = document.getElementById(`tax-row-${newlyAddedId}`);
      if (element) {
  ...
  // Fetch titles when taxes change
  useEffect(() => {
    if (defaultTaxes.length > 0 && !defaultTaxes[0].level4Title) {
      fetchTitlesForTaxes();
    }
  }, [defaultTaxes.length]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (rate && isNaN(parseFloat(rate))) {
      setMessage({
        type: "error",
        text: "Rate must be a numeric value"
      });
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      const url = editId
        ? `http://localhost:5000/api/defaults/govt_taxes/${editId}`
        : `http://localhost:5000/api/defaults/govt_taxes`;
      ...
      setMessage({
        type: "success",
        text: editId ? "Government tax account updated successfully!" : "Government tax account added successfully!"
      });
      
      // Keep level 1, level 2, and level 3 selections after adding/updating
      if (!editId) {
        // Only reset level 4 and rate when adding new
        setLevel4Id('');
        setRate('');
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this government tax account?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/defaults/govt_taxes/${id}`, {
          method: 'DELETE'
        });
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Failed to delete government tax account');
        
        setMessage({
          type: "success",
          text: "Government tax account deleted successfully!"
        });
        fetchDefaultTaxes();
      } catch (err) {
        setMessage({
          type: "error",
          text: err.message || 'Failed to delete government tax account'
        });
      } finally {
        setLoading(false);
      }
  ...
  const toggleActive = async (id, currentStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/govt_taxes/${id}/active`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const toggleDefault = async (id, currentStatus) => {
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:5000/api/defaults/govt_taxes/${id}/default`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
    ...
    <>
      <style jsx global>{`
        ...
        @media (max-width: 768px) {
          ...
          /* Adjust rate input width on mobile */
          .rate-input {
            min-width: 100%;
          }
        }
      ...
      <div className="ml-0 md:ml-64 transition-all duration-300 fixed inset-0 overflow-hidden">
        <div className="max-w-4xl mx-auto mt-6 p-4 md:p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg flex flex-col h-[calc(100vh-3rem)] md:h-[calc(100vh-3rem)]">
          <h2 className="text-xl md:text-2xl font-bold text-center text-blue-600 dark:text-blue-400 mb-4 md:mb-6 flex items-center justify-center gap-2">
            ...
            <span className="truncate">Default Government Taxes</span>
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className={`${inputClass} rate-input`}
                step="0.01"
                min="0"
                max="100"
                required
            ...
            >
              ...
              {editId ? 'Update Government Tax' : 'Add Government Tax'}
            </button>
          </form>
          ...
          {companyId ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar responsive-table">
                {defaultTaxes.length > 0 ? (
                  <table className="w-full">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200">
                        ...
                        <th className="p-3 text-left font-semibold text-sm">Rate</th>
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No default government tax accounts found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view default government tax accounts
            </div>
          )}
        </div>
      </div>
