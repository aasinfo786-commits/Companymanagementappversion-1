...
export default function DefaultBrokerAccount() {
  ...
  const [defaultBrokerAccounts, setDefaultBrokerAccounts] = useState([]);
  ...
  // Fetch default broker accounts with improved empty state handling
  const fetchDefaultBrokerAccounts = async () => {
    if (!companyId) {
      setDefaultBrokerAccounts([]);
      return;
    }
    setLoading(true);
    setMessage(null);
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch default broker accounts');
      }
      const responseData = await res.json();
      
      // Handle both array and object response formats
      const brokerAccountsData = Array.isArray(responseData) ? responseData : (responseData.data || []);
      setDefaultBrokerAccounts(brokerAccountsData);
      // Show info message if empty
      if (brokerAccountsData.length === 0) {
        setMessage({
          type: "info",
          text: "No default broker accounts found. You can add one using the form above."
        });
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (newlyAddedId && tableRef.current) {
      const element = document.getElementById(`broker-row-${newlyAddedId}`);
      if (element) {
  ...
  }, [newlyAddedId, defaultBrokerAccounts]);
  
  useEffect(() => {
    fetchLevel1Accounts();
    fetchDefaultBrokerAccounts();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const existingBrokerAccount = defaultBrokerAccounts.find(brokerAccount => 
        brokerAccount.level1Id === level1Id && 
        brokerAccount.level2Id === level2Id && 
        brokerAccount.level3Id === level3Id && 
        brokerAccount._id !== editId
      );
      ...
      if (existingBrokerAccount) {
        ...
        if (editId) {
          ...
          await fetch(`http://localhost:5000/api/defaults/brokerAccounts/${editId}`, {
            method: 'DELETE'
          });
          ...
          setMessage({
            type: "success",
            text: "Default broker account updated successfully!"
          });
        } else {
          // If we're adding a new record and it already exists
          throw new Error("This combination of accounts already exists as a default broker account");
        }
      }
      ...
      const url = editId 
        ? `http://localhost:5000/api/defaults/brokerAccounts/${editId}` 
        : `http://localhost:5000/api/defaults/brokerAccounts`;
      ...
      if (!existingBrokerAccount) {
        setMessage({
          type: "success",
          text: editId ? "Default broker account updated successfully!" : "Default broker account added successfully!"
        });
      }
      ...
      setEditId(null);
      await fetchDefaultBrokerAccounts();
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this default broker account?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/defaults/brokerAccounts/${id}`, {
          method: 'DELETE'
        });
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Failed to delete default broker account');
        setMessage({
          type: "success",
          text: "Default broker account deleted successfully!"
        });
        fetchDefaultBrokerAccounts();
      } catch (err) {
        setMessage({
          type: "error",
          text: err.message || 'Failed to delete default broker account'
        });
      } finally {
        setLoading(false);
      }
  ...
  const toggleActive = async (id, currentStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/brokerAccounts/${id}/active`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const toggleDefault = async (id, currentStatus) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/defaults/brokerAccounts/${id}/default`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        ...
        <div className="max-w-4xl mx-auto mt-6 p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg flex flex-col h-[calc(100vh-3rem)]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6 flex-shrink-0">
            ...
            >
              ...
              {editId ? 'Update Default Broker Account' : 'Add as Default Broker Account'}
            </button>
          </form>
          ...
          {companyId ? (
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl shadow-md overflow-hidden flex-grow flex flex-col max-h-[35vh]">
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar">
                {defaultBrokerAccounts.length > 0 ? (
                  <table className="w-full">
                    ...
                    <tbody>
                      {defaultBrokerAccounts.map((item) => {
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No default broker accounts found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view default broker accounts
            </div>
          )}
        </div>
      </div>
