...
export default function DefaultCash() {
  ...
  // Fetch default cash accounts with improved empty state handling
  const fetchDefaultCash = async () => {
    if (!companyId) {
      setDefaultCash([]);
      return;
    }
    ...
    setMessage(null);
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/cash/${companyId}`);
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch default cash accounts');
      }
      const responseData = await res.json();
      // Handle both array and object response formats
      const cashData = Array.isArray(responseData) ? responseData : (responseData.data || []);
      setDefaultCash(cashData);
      // Show info message if empty
      if (cashData.length === 0) {
        setMessage({
          type: "info",
          text: "No default cash accounts found. You can add one using the form above."
        });
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (newlyAddedId && tableRef.current) {
      const element = document.getElementById(`cash-row-${newlyAddedId}`);
      if (element) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const existingCash = defaultCash.find(cash => 
        cash.level1Id === level1Id && 
        cash.level2Id === level2Id && 
        cash.level3Id === level3Id && 
        cash._id !== editId
      );
      ...
      if (existingCash) {
        ...
        if (editId) {
          ...
          await fetch(`http://localhost:5000/api/defaults/cash/${editId}`, {
            method: 'DELETE'
          });
          ...
          setMessage({
            type: "success",
            text: "Default cash account updated successfully!"
          });
        } else {
          // If we're adding a new record and it already exists
          throw new Error("This combination of accounts already exists as a default cash account");
        }
      }
      ...
      const url = editId
        ? `http://localhost:5000/api/defaults/cash/${editId}`
        : `http://localhost:5000/api/defaults/cash`;
      ...
      if (!existingCash) {
        setMessage({
          type: "success",
          text: editId ? "Default cash account updated successfully!" : "Default cash account added successfully!"
        });
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this default cash account?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/defaults/cash/${id}`, {
          method: 'DELETE'
        });
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Failed to delete default cash account');
        setMessage({
          type: "success",
          text: "Default cash account deleted successfully!"
        });
        fetchDefaultCash();
      } catch (err) {
        setMessage({
          type: "error",
          text: err.message || 'Failed to delete default cash account'
        });
      } finally {
        setLoading(false);
      }
  ...
  const toggleActive = async (id, currentStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/cash/${id}/active`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const toggleDefault = async (id, currentStatus) => {
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:5000/api/defaults/cash/${id}/default`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        ...
        <option key={account._id} value={account._id}>
          {account.code} - {account.title}
            ...
            >
              ...
              {editId ? 'Update Default Cash Account' : 'Add as Default Cash Account'}
            </button>
          </form>
          ...
          {companyId ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar">
                {defaultCash.length > 0 ? (
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No default cash accounts found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view default cash accounts
            </div>
          )}
        </div>
      </div>
