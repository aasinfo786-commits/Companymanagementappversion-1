...
export default function DefaultBanks() {
  ...
  const [newlyAddedId, setNewlyAddedId] = useState(null); // Added for highlighting new entries
  const tableRef = useRef(null); // Added for table reference
  ...
  // Fetch default banks with improved empty state handling
  const fetchDefaultBanks = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/banks/${companyId}`);
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch default banks');
      }
      const responseData = await res.json();
      // Handle both array and object response formats
      const banksData = Array.isArray(responseData) ? responseData : (responseData.data || []);
      setDefaultBanks(banksData);
      // Show info message if empty
      if (banksData.length === 0) {
        setMessage({
          type: "info",
          text: "No default banks found. You can add one using the form above."
        });
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (newlyAddedId && tableRef.current) {
      const element = document.getElementById(`bank-row-${newlyAddedId}`);
      if (element) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const existingBank = defaultBanks.find(bank => 
        bank.level1Id === level1Id && 
        bank.level2Id === level2Id && 
        bank.level3Id === level3Id && 
        bank._id !== editId
      );
      ...
      if (existingBank) {
        ...
        if (editId) {
          ...
          await fetch(`http://localhost:5000/api/defaults/banks/${editId}`, {
            method: 'DELETE'
          });
          ...
          setMessage({
            type: "success",
            text: "Default bank updated successfully!"
          });
        } else {
          // If we're adding a new record and it already exists
          throw new Error("This combination of accounts already exists as a default bank");
        }
      }
      ...
      const url = editId
        ? `http://localhost:5000/api/defaults/banks/${editId}`
        : `http://localhost:5000/api/defaults/banks`;
      ...
      if (!existingBank) {
        setMessage({
          type: "success",
          text: editId ? "Default bank updated successfully!" : "Default bank added successfully!"
        });
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this default bank?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/defaults/banks/${id}`, {
          method: 'DELETE'
        });
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Failed to delete default bank');
        setMessage({
          type: "success",
          text: "Default bank deleted successfully!"
        });
        fetchDefaultBanks();
      } catch (err) {
        setMessage({
          type: "error",
          text: err.message || 'Failed to delete default bank'
        });
      } finally {
        setLoading(false);
      }
  ...
  const toggleActive = async (id, currentStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/banks/${id}/active`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const toggleDefault = async (id, currentStatus) => {
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:5000/api/defaults/banks/${id}/default`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
    ...
    <>
      ...
      <div className="ml-0 md:ml-64 transition-all duration-300 fixed inset-0 overflow-hidden">
        <div className="max-w-4xl mx-auto mt-6 p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg flex flex-col h-[calc(100vh-3rem)]">
          ...
          {message && (
            <div
              className={`mb-4 p-3 rounded-xl text-white font-medium shadow-md flex items-center gap-2 ${message.type === "success" ? "bg-green-500" :
  ...
  {/* Row 1: Level 1 and Level 2 side by side */}
  <div className="grid grid-cols-2 gap-4">
    <div className="flex items-center gap-2 w-full">
      <ListTree className="text-blue-500 w-4 h-4 shrink-0" />
      <select
        value={level1Id}
        onChange={(e) => {
    ...
    <div className="flex items-center gap-2 w-full">
      <Layers className="text-blue-500 w-4 h-4 shrink-0" />
      <select
        value={level2Id}
        onChange={(e) => {
        ...
        <option key={account._id} value={account._id}>
          {account.code} - {account.title}
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl font-semibold text-base flex items-center justify-center gap-2 shadow-md transition ${editId
            ...
            >
              ...
              {editId ? 'Update Default Bank' : 'Add as Default Bank'}
            </button>
          </form>
          ...
          {companyId ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar">
                {defaultBanks.length > 0 ? (
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No default banks found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view default banks
            </div>
          )}
        </div>
      </div>
