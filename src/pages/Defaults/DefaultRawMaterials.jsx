...
export default function DefaultRawMaterials() {
  ...
  const [defaultRawMaterials, setDefaultRawMaterials] = useState([]);
  ...
  // Fetch default raw materials with improved empty state handling
  const fetchDefaultRawMaterials = async () => {
    if (!companyId) {
      setDefaultRawMaterials([]);
      return;
    }
    setLoading(true);
    setMessage(null);
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch default raw materials');
      }
      const responseData = await res.json();
      
      // Handle both array and object response formats
      const rawMaterialsData = Array.isArray(responseData) ? responseData : (responseData.data || []);
      setDefaultRawMaterials(rawMaterialsData);
      
      // Show info message if empty
      if (rawMaterialsData.length === 0) {
        setMessage({
          type: "info",
          text: "No default raw materials found. You can add one using the form above."
        });
      }
    } catch (err) {
  ...
  // Fetch level 3 accounts for all raw materials to get their titles
  const fetchTitlesForRawMaterials = async () => {
    if (defaultRawMaterials.length === 0) return;
    
    try {
      // Get all unique level 3 IDs from raw materials
      const level3Ids = [...new Set(defaultRawMaterials.map(rm => rm.level3Id))];
      
      // Try to fetch level 3 accounts for these IDs
      const promises = level3Ids.map(id => 
        fetch(`http://localhost:5000/api/accounts/single/${id}`)
          .then(res => {
      ...
      const level3AccountMap = {};
      
      accounts.forEach(account => {
        if (account && account._id) {
          level3AccountMap[account._id] = account.title;
        }
      });
      
      // Update raw materials with titles
      setDefaultRawMaterials(prev => 
        prev.map(rm => ({
          ...rm,
          title: level3AccountMap[rm.level3Id] || rm.title || 'Unknown Title'
        }))
      );
    } catch (err) {
      console.error('Error fetching titles for raw materials:', err);
      // If fetching fails, keep existing titles or set to 'Unknown Title'
      setDefaultRawMaterials(prev => 
        prev.map(rm => ({
          ...rm,
          title: rm.title || 'Unknown Title'
        }))
      );
    }
  };
  ...
  useEffect(() => {
    if (newlyAddedId && tableRef.current) {
      const element = document.getElementById(`raw-row-${newlyAddedId}`);
      if (element) {
  ...
  }, [newlyAddedId, defaultRawMaterials]);
  
  useEffect(() => {
    fetchLevel1Accounts();
    fetchDefaultRawMaterials();
  }, [companyId]);
  ...
  // Fetch titles when raw materials change
  useEffect(() => {
    if (defaultRawMaterials.length > 0 && !defaultRawMaterials[0].title) {
      fetchTitlesForRawMaterials();
    }
  }, [defaultRawMaterials.length]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editId 
        ? `http://localhost:5000/api/defaults/rawMaterials/${editId}` 
        : `http://localhost:5000/api/defaults/rawMaterials`;
      ...
      setMessage({
        type: "success",
        text: editId ? "Default raw material updated successfully!" : "Default raw material added successfully!"
      });
      
      // Keep level 1 and level 2 selections after adding/updating
      if (!editId) {
        // Only reset level 3 when adding new
        setLevel3Id('');
        // Set the newly added ID to highlight it
        setNewlyAddedId(json._id || json.data?._id);
      }
      setEditId(null);
      await fetchDefaultRawMaterials();
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this default raw material?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/defaults/rawMaterials/${id}`, {
          method: 'DELETE'
        });
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Failed to delete default raw material');
        
        setMessage({
          type: "success",
          text: "Default raw material deleted successfully!"
        });
        fetchDefaultRawMaterials();
      } catch (err) {
        setMessage({
          type: "error",
          text: err.message || 'Failed to delete default raw material'
        });
      } finally {
        setLoading(false);
      }
  ...
  const toggleActive = async (id, currentStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/rawMaterials/${id}/active`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const toggleDefault = async (id, currentStatus) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/defaults/rawMaterials/${id}/default`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        ...
        <option key={account._id} value={account._id}>
          {account.code} - {account.title}
            ...
            >
              ...
              {editId ? 'Update Default Raw Material' : 'Add as Default Raw Material'}
            </button>
          </form>
          ...
          {companyId ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar">
                {defaultRawMaterials.length > 0 ? (
                      ...
                      {defaultRawMaterials.map((item) => {
                        return (
                          ...
                          >
                            ...
                            <td className="p-3 text-sm">{item.title || 'Unknown Title'}</td>
                            <td className="p-3">
                              <button
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No default raw materials found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view default raw materials
            </div>
          )}
        </div>
      </div>
