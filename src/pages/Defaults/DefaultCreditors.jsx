...
export default function DefaultCreditors() {
  ...
  const [defaultCreditors, setDefaultCreditors] = useState([]);
  ...
  // Fetch default creditors with improved empty state handling
  const fetchDefaultCreditors = async () => {
    if (!companyId) {
      setDefaultCreditors([]);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch default creditors');
      }
      const responseData = await res.json();
      
      // Handle both array and object response formats
      const creditorsData = Array.isArray(responseData) ? responseData : (responseData.data || []);
      setDefaultCreditors(creditorsData);
      // Show info message if empty
      if (creditorsData.length === 0) {
        setMessage({
          type: "info",
          text: "No default creditors found. You can add one using the form above."
        });
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (newlyAddedId && tableRef.current) {
      const element = document.getElementById(`creditor-row-${newlyAddedId}`);
      if (element) {
  ...
  }, [newlyAddedId, defaultCreditors]);
  
  useEffect(() => {
    fetchLevel1Accounts();
    fetchDefaultCreditors();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const existingCreditor = defaultCreditors.find(creditor => 
        creditor.level1Id === level1Id && 
        creditor.level2Id === level2Id && 
        creditor.level3Id === level3Id && 
        creditor._id !== editId
      );
      ...
      if (existingCreditor) {
        ...
        if (editId) {
          ...
          await fetch(`http://localhost:5000/api/defaults/creditors/${editId}`, {
            method: 'DELETE'
          });
          ...
          setMessage({
            type: "success",
            text: "Default creditor updated successfully!"
          });
        } else {
          // If we're adding a new record and it already exists
          throw new Error("This combination of accounts already exists as a default creditor");
        }
      }
      ...
      const url = editId 
        ? `http://localhost:5000/api/defaults/creditors/${editId}` 
        : `http://localhost:5000/api/defaults/creditors`;
      ...
      if (!existingCreditor) {
        setMessage({
          type: "success",
          text: editId ? "Default creditor updated successfully!" : "Default creditor added successfully!"
        });
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this default creditor?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/defaults/creditors/${id}`, {
          method: 'DELETE'
        });
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Failed to delete default creditor');
        setMessage({
          type: "success",
          text: "Default creditor deleted successfully!"
        });
        fetchDefaultCreditors();
      } catch (err) {
        setMessage({
          type: "error",
          text: err.message || 'Failed to delete default creditor'
        });
      } finally {
        setLoading(false);
      }
  ...
  const toggleActive = async (id, currentStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/creditors/${id}/active`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const toggleDefault = async (id, currentStatus) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/defaults/creditors/${id}/default`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        ...
        <option key={account._id} value={account._id}>
          {account.code} - {account.title}
            ...
            >
              ...
              {editId ? 'Update Default Creditor' : 'Add as Default Creditor'}
            </button>
          </form>
          ...
          {companyId ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar">
                {defaultCreditors.length > 0 ? (
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No default creditors found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view default creditors
            </div>
          )}
        </div>
      </div>
