...
export default function DefaultDebtors() {
  ...
  const [defaultDebtors, setDefaultDebtors] = useState([]);
  ...
  // Fetch default debtors with improved empty state handling
  const fetchDefaultDebtors = async () => {
    if (!companyId) {
      setDefaultDebtors([]);
      return;
    }
    ...
    setMessage(null);
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/debtors/${companyId}`);
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch default debtors');
      }
      const responseData = await res.json();
      
      // Handle both array and object response formats
      const debtorsData = Array.isArray(responseData) ? responseData : (responseData.data || []);
      setDefaultDebtors(debtorsData);
      // Show info message if empty
      if (debtorsData.length === 0) {
        setMessage({
          type: "info",
          text: "No default debtors found. You can add one using the form above."
        });
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (newlyAddedId && tableRef.current) {
      const element = document.getElementById(`debtor-row-${newlyAddedId}`);
      if (element) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const existingDebtor = defaultDebtors.find(debtor => 
        debtor.level1Id === level1Id && 
        debtor.level2Id === level2Id && 
        debtor.level3Id === level3Id && 
        debtor._id !== editId
      );
      ...
      if (existingDebtor) {
        ...
        if (editId) {
          ...
          await fetch(`http://localhost:5000/api/defaults/debtors/${editId}`, {
            method: 'DELETE'
          });
          ...
          setMessage({
            type: "success",
            text: "Default debtor updated successfully!"
          });
        } else {
          // If we're adding a new record and it already exists
          throw new Error("This combination of accounts already exists as a default debtor");
        }
      }
      ...
      const url = editId 
        ? `http://localhost:5000/api/defaults/debtors/${editId}` 
        : `http://localhost:5000/api/defaults/debtors`;
      ...
      if (!existingDebtor) {
        setMessage({
          type: "success",
          text: editId ? "Default debtor updated successfully!" : "Default debtor added successfully!"
        });
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this default debtor?')) {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/defaults/debtors/${id}`, {
          method: 'DELETE'
        });
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Failed to delete default debtor');
        setMessage({
          type: "success",
          text: "Default debtor deleted successfully!"
        });
        fetchDefaultDebtors();
      } catch (err) {
        setMessage({
          type: "error",
          text: err.message || 'Failed to delete default debtor'
        });
      } finally {
        setLoading(false);
      }
  ...
  const toggleActive = async (id, currentStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/defaults/debtors/${id}/active`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const toggleDefault = async (id, currentStatus) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/defaults/debtors/${id}/default`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        ...
        <option key={account._id} value={account._id}>
          {account.code} - {account.title}
            ...
            >
              ...
              {editId ? 'Update Default Debtor' : 'Add as Default Debtor'}
            </button>
          </form>
          ...
          {companyId ? (
              ...
              <div ref={tableRef} className="overflow-y-auto flex-grow custom-scrollbar">
                {defaultDebtors.length > 0 ? (
                  ...
                  <div className="p-6 text-center text-gray-500 dark:text-gray-400">
                    {message?.type !== "error" && "No default debtors found. Add one using the form above."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 text-center text-gray-500 dark:text-gray-400 flex-grow">
              Please select a company to view default debtors
            </div>
          )}
        </div>
      </div>
